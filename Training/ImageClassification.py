...
class TransferLearning:
    class Params:
        ...
        def get_output_activation_functions(self):
            """
        ...
        class dominate_params_file:
            def __init__(self, params_file_path, train_folder):
                ...
                try:
                    with open(params_file_path, "r") as p:
                        (
                            all_params,
                            skiped_models,
                            epochs,
            ...
            def update_data_path(self, new_data_path):
                ...
                with open(self.params_file_path, "r") as p:
            ...
            def split(self, divide: int = 2):
                ...
                with open(self.params_file_path) as p:
                    (
                        all_params,
                        skiped_models,
                        epochs,
    ...
    def _save_unused_params(
        ...
        if not split:
            with open(
                os.path.join(save_to_path, model_name + "_unused_params.txt"), "w"
            ) as f:
            ...
            with open(
                os.path.join(save_to_path, model_name + params_file_end + ".txt"), "w"
            ) as f:
                f.write(str(params).replace("'", ""))
    ...
    def _write_to_log_file(text, save_to_path):# type: ignore
        ...
        text = text + "=" * 83 + "\n"
        with open(os.path.join(save_to_path, "log.txt"), mode) as log_file:
            if mode == "a":
                log_file.write("\n" + text + "\n")
            else:
                log_file.write(text + "\n")
        ...
        while len(all_params) != 0:
            ...
            try:
                ...
                if model is None and history is None:
                    ...
                    del all_params[0]
                    __save_unused_params(save_data_paths)# type: ignore
                    del save_data_paths # type: ignore
                    continue
                print("Testing Model 🥼")
